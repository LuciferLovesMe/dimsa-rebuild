...
@section('content')
    <a href="{{ $backUrl }}" class="mb-4 inline-flex items-center text-sm font-light text-gray-500 hover:text-gray-800">
        <i class="fa fa-arrow-left mr-1"></i>
        Kembali
    </a>

    <div class="p-8 mt-4 min-h-96 rounded-lg shadow-lg bg-white" x-data="dewanForm({
        dewanId: {{ $dewan->id }},
        fetchUrl: '{{ $fetchUrl }}',
        updateUrl: '{{ $updateUrl }}',
        redirectUrl: '{{ $backUrl }}'
        ...
        <div x-show="!isFetchingData" style="display: none;">
            <p class="text-xl font-bold text-gray-800">{{ $pageTitle }}</p>
            <hr class="my-6">

            <form @submit.prevent="confirmSubmit" id="dewan-form" enctype="multipart/form-data" method="post">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                            ...
                            <div class="space-y-6">
                                <div>
                                    ...
                                    <input type="text" name="nama" id="nama"
                                        ...
                                        placeholder="Nama Lengkap" required value="{{ $dewan->nama }}">
                                </div>
                                <div>
                                    ...
                                    <input type="text" name="jabatan" id="jabatan"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                        placeholder="Contoh: Ketua Dewan" required value="{{ $dewan->jabatan }}">
                                </div>
                            </div>
                        </div>
                        <hr />
                    ...
                    {{-- KOLOM KANAN --}}
                    <div class="space-y-6">
                        <x-input.image-uploader name="image" :src="$dewan->image_url" />
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between mt-8 pt-6 border-t">
                    <div class="flex items-center">
                        <input id="is_publish" name="is_publish" type="checkbox" value="1"
                            ...
                            {{ $dewan->is_publish ? 'checked' : '' }}>
                        <label for="is_publish" class="ml-2 text-sm text-gray-900">*Centang untuk mempublikasikan</label>
                    </div>
                    <button type="submit"
            ...
            return {
                ...
                async init() {
                    ...
                    try {
                        ...
                        Swal.fire('Gagal', 'Tidak dapat memuat data relasi.', 'error');
                    } finally {
                ...
                saveItem(type) {
                    const isDraftValid = Object.values(this.drafts[type]).some((val) => val && val.toString().trim() !==
                        "");
                    if (!isDraftValid) {
                        Swal.fire({
                            icon: "warning",
                ...
                async submitForm() {
                    this.isLoading = true;
                    const formElement = document.getElementById("dewan-form");
                    ...
                    try {
                        ...
                        if (response.ok) {
                            ...
                            Swal.fire({
                                ...
                                html: result.message + (result.errors ? "<br><br>" + Object.values(result
                                    .errors).map(e => e.join('<br>')).join("<br>") : ""),
                            });
                        }
                    } catch (error) {
                        console.error("Fetch error:", error);
