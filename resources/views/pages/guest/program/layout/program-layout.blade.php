...
@section('content')
    ...
    <div class="relative h-screen bg-cover bg-center"
        style="background-image: url('{{ $heroImage ?? 'https://placehold.co/1920x1080/2d3748/e2e8f0/' }}');">
        ...
        <div class="relative h-full flex items-center justify-start p-4 sm:p-8 lg:p-20">
            <div class="w-full max-w-md md:max-w-xl lg:max-w-2xl">
                ...
                <p class="mt-5 text-base md:text-lg text-white leading-relaxed text-justify">
                    {{ $heroDesc ?? 'Deskripsi Halaman Program' }}
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-gray-100 py-12 md:py-20 px-4 sm:p-8 lg:p-20">
        <div>
            <h2 class="text-3xl md:text-4xl font-bold text-gray-800 text-center mb-8">{{ $videoTitle ?? 'Profil Sekolah' }}
            </h2>
            {{-- Responsive Video Wrapper with inline CSS for aspect ratio --}}
            <div class="relative rounded-lg overflow-hidden shadow-xl" style="padding-top: 56.25%; /* 16:9 Aspect Ratio */">
                ...
                @else
                    {{-- Placeholder jika tidak ada URL video --}}
