...
<div>
    <div class="flex justify-between items-center">
        ...
        <button @click="openForm('pendidikan')" type="button"
            ...
            :class="{ 'transform rotate-45': openSections.pendidikan }">
    ...
    <div class="mt-4 space-y-2">
        <template x-for="(item, index) in formData.pendidikan" :key="index">
            <div class="border rounded-md p-3 flex justify-between items-center bg-white">
                <div>
                    <p class="font-semibold" x-text="item.tingkat_pendidikan"></p>
                    <p class="text-sm text-gray-500"
                        x-text="`${item.instansi} (${item.tahun_mulai} - ${item.tahun_akhir})`">
                    </p>
                </div>
    ...
    <div x-show="openSections.pendidikan" class="mt-4 p-5 bg-gray-50 rounded-lg shadow-inner">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
            <div class="sm:col-span-2">
                <label for="tingkat_pendidikan" class="block text-sm font-medium text-gray-700">Tingkat
                    Pendidikan</label>
                {{-- REVISI: Mengubah input menjadi dropdown (select) agar sesuai dengan ENUM --}}
                <select id="tingkat_pendidikan" x-model="drafts.pendidikan.tingkat_pendidikan"
            ...
            <div class="sm:col-span-2">
                ...
                <input type="text" x-model="drafts.pendidikan.instansi" placeholder="Tulis Nama Instansi"
                    class="mt-1 bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700">Tahun Mulai</label>
                <input type="number" x-model="drafts.pendidikan.tahun_mulai" placeholder="Contoh: 2018"
                    class="mt-1 bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700">Tahun Berakhir</label>
                <input type="number" x-model="drafts.pendidikan.tahun_akhir" placeholder="Contoh: 2022"
                    class="mt-1 bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
            </div>
        </div>
        <div class="flex justify-end gap-3 mt-6">
            ...
            <button @click="saveItem('pendidikan')" type="button"
                class="p-2.5 text-white bg-gray-800 border border-transparent rounded-lg shadow-sm hover:bg-gray-900"
